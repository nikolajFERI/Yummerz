...
<div bind:this={dragZone} on:dragover={dragover} on:drop={drop} on:dragenter={dragenter} on:dragleave={dragleave} class={`${fixed ? 'fixed' : ''} fileUploader dragzone`}>
	{#if files.length !== maxFiles}
	  {#if listFiles}
		...
		{/if}
		<div class="buttons">
			<button on:click={trigger} class="secondary">
				{buttonText}
			</button>
			{#if doneButtonText && files.length}<button on:click={() => (doneCallback())}>{doneButtonText}</button>{/if}
		</div>
		{#if descriptionText}<span class="text">{descriptionText}</span>{/if}
	{:else if maxFiles > 1 || doneBoolean === true}
